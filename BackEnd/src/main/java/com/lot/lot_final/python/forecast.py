...
'''
# ------------------------------------------------------------------------------------------------------------------------------------
with open("./loTData/data.json", "r") as fin:
    data = json.load(fin)
test_data = data['data']
test_target = data['target']

test_ans = list(forecast_by_Logistic(test_data, test_target))

with open("./loTData/ans.json", 'w') as fout:
    json.dump({"ans": json.dumps(test_ans)}, fout)
